...
o	In Objects > Security Profiles > Vulnerability Protection.
o	Add a New Profile:
o	Named the profile (e.g., Nmap_Scan_Detection)
Added Signature Exceptions for Nmap Detection:
o	Selected multiple Nmap related signatures
o	Set the Action to Block for each of these signatures.
o	Ensure that logging is enabled for the new security policy rule.
Verify and Tested
o	Checked that Monitor > Logs > Threat to review the alerts generated by Nmap scan detections.
 
...
Identifies misconfigurations such as directory indexing, HTTP methods enabled (like PUT, DELETE), and default files (e.g., index.php, index.html).
...
o	In Objects > Security Profiles > Vulnerability Protection.
o	Add a New Profile:
o	Named the profile  Nikto_Scan_Detection)
Added Signature Exceptions for Nikto Detection:
o	Selected any Nikto related signatures
o	Set the Action to Block for each of these signatures.
o	Ensure that logging is enabled for the new security policy rule.
Verify and Tested
o	Checked that Monitor > Logs > Threat to review the alerts generated by Nmap scan detections.
...
 
Heartbleed
sf6 exploit(multi/http/apache_mod_cgi_bash_env_exec) > back
